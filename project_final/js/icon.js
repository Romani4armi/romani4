...
var kolProdukt = document.getElementById('kvoProdukt');
    ...
    function sneakerBrend(a){ //фильтр по товарам
        var arrBrendCheckbox;
       if (localStorage.brendCheckbox.split(',').includes(a)){
            arrBrendCheckbox = localStorage.brendCheckbox.split(',')
            
            for(var i=0; i<arrBrendCheckbox.length;i++){
                if (arrBrendCheckbox[i]==a) {
                    arrBrendCheckbox[i]=''
                }
                // if(arrBrendCheckbox[i]==''){
                //     arrBrendCheckbox.splice((i-1),1);
                    
                // }
            }
          

            localStorage.brendCheckbox=arrBrendCheckbox.join(',');
           
       } else{
       localStorage.brendCheckbox+=a + ',';
        arrBrendCheckbox =  localStorage.brendCheckbox.split(',');
      
       
       arrBrendCheckbox.pop();
      html=``;
       for(var i=0; i<sneakersArr.length; i++){
            if(arrBrendCheckbox.includes(sneakersArr[i].brend)){
                console.log('не должно зайти')
        sneakersArr[i].newSneker==true?newSnekerV='style="display:block"':newSnekerV='';
        sneakersArr[i].hit==true?hitTovar='style="display:block"':hitTovar='';
        sneakersArr[i].discount>0?discountSneaker='style="display:block"':discountSneaker='';
